...
(function () {
  ...
  var LobiboxBase = {
    ...
    _createMarkup: function () {
      var me = this;
      var lobibox = $('<div className="lobibox"></div>');
      lobibox.attr('data-is-modal', me.$options.modal);
      var header = $('<div className="lobibox-header"></div>').append(
        '<span className="lobibox-title"></span>'
      );
      var body = $('<div className="lobibox-body"></div>');
      lobibox.append(header);
      lobibox.append(body);
      if (me.$options.buttons && !$.isEmptyObject(me.$options.buttons)) {
        var footer = $('<div className="lobibox-footer"></div>');
    ...
    _addBackdrop: function () {
      if ($('.lobibox-backdrop').length === 0) {
        $('body').append('<div className="lobibox-backdrop"></div>');
      }
    },
    ...
    _addCloseButton: function () {
      var me = this;
      var closeBtn = $('<span className="btn-close">&times;</span>');
      ...
      if (me.$options.required && !me.getValue()) {
        ...
        if ($error.length === 0) {
          me.$el
            .find('.lobibox-body')
            .append(
              '<p className="lobibox-input-error-message">' +
  ...
  LobiboxConfirm.prototype = $.extend({}, LobiboxBase, {
    ...
    _afterShow: function () {
      ...
      if (me.$options.iconClass) {
        d.append(
          $('<div className="lobibox-icon-wrapper"></div>').append(
            '<i className="lobibox-icon ' + me.$options.iconClass + '"></i>'
          )
        );
      }
      d.append(
        '<div className="lobibox-body-text-wrapper"><span className="lobibox-body-text">' +
          me.$options.msg +
          '</span></div>'
      );
  ...
  LobiboxProgress.prototype = $.extend({}, LobiboxBase, {
    ...
    _createProgressbar: function () {
      var me = this;
      var outer = $(
        '<div className="lobibox-progress-bar-wrapper lobibox-progress-outer"></div>'
      ).append(
        '<div className="lobibox-progress-bar lobibox-progress-element"></div>'
      );
      if (me.$options.showProgressLabel) {
        outer.append(
          '<span className="lobibox-progress-text" data-role="progress-text"></span>'
        );
      }
  ...
  var LobiboxNotify = function (type, options) {
    ...
    var _createTabControl = function (tabPaneId) {
      ...
      $('<a></a>', {
        ...
        .append('<i className="tab-control-icon ' + me.$options.icon + '"></i>')
        .appendTo($li);
      return $li;
    };
    ...
    var _createNotifyWrapper = function () {
      ...
      if ($wrapper.length === 0) {
        ...
        if (me.$options.size === 'large') {
          $wrapper
            .append($('<ul className="lb-notify-tabs"></ul>'))
            .append($('<div className="lb-notify-wrapper"></div>'));
        }
      }
      return $wrapper;
    };
    var _createNotify = function () {
      ...
      $iconWrapper = $(
        '<div className="lobibox-notify-icon-wrapper"></div>'
      ).appendTo($notify);
      $iconEl = $('<div className="lobibox-notify-icon"></div>').appendTo(
        $iconWrapper
      );
      ...
      if (me.$options.img) {
        ...
        $innerIconEl.append(
          '<div className="icon-el"><i className="' +
            me.$options.icon +
            '"></i></div>'
        );
      } else {
      ...
      $msg = $(
        '<div className="lobibox-notify-msg">' + me.$options.msg + '</div>'
      );
      ...
      if (me.$options.title) {
        $body.prepend(
          '<div className="lobibox-notify-title">' + me.$options.title + '<div>'
        );
      }
    ...
    var _addCloseButton = function ($el) {
      ...
      $('<span className="lobibox-close">&times;</span>')
        .click(function (ev) {
          ev.preventDefault();
          ev.stopPropagation();
          me.remove();
    ...
    var _addDelay = function ($el) {
      ...
      if (me.$options.delayIndicator) {
        var delay = $(
          '<div className="lobibox-delay-indicator"><div></div></div>'
        );
        $el.append(delay);
      }
      var time = 0;
